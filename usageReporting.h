...
// Simple-Calculator was started, and nothing else.
//
// One event, `startup`, tagged with the version; it carries the program name
// and nothing about the person running it, their machine or your calculations. It is
...
static const char *const PROGRAM_NAME = "Simple-Calculator";
...
/**
 * Where the settings file lives: %APPDATA%\Simple-Calculator on Windows,
 * ~/Library/Application Support/Simple-Calculator on macOS, and
 * $XDG_CONFIG_HOME/Simple-Calculator (default ~/.config/Simple-Calculator) elsewhere.
...
inline std::string settingsFileContent() {
    return std::string("# ") + PROGRAM_NAME + " usage reporting - "
        ...
        "# your calculations. Set enabled to false to turn it off. TRACE_USAGE_REPORTING=off\n"
...
inline std::string notice(const std::string &path) {
    ...
    return std::string("Usage reporting is on: ") + PROGRAM_NAME
        + " sends a startup event (its name and version) to https://trace.danielstephenson.dev"
          " - nothing about you, your machine or your calculations. Turn it off with enabled=false in "
...
#if defined(SIMPLE_CALCULATOR_VERSION)
    return SIMPLE_CALCULATOR_VERSION;
#else
